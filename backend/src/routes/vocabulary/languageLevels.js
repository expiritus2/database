...
router.put('/api/vocabulary/languageLevels', middlewares, async(req, res) => {
    await LanguageLevel.destroy({ where: {}});
    const items = await LanguageLevel.bulkCreate(req.body);
    res.send(items);
});
