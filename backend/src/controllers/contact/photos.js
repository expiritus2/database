...
class Photos {
    ...
    delete(contactId) {
        return new Promise(async (resolve) => {
            const photos = await Photo.findAll({ where: { contactId }});
    ...
    #createPhotos(photos) {
        return new Promise(async (resolve) => {
            ...
            for await (const photo of uploadedPhotos) {
                ...
                await newPhoto.setContact(this.contact);
            }
    ...
    #deleteRemovedPhotos() {
        return new Promise(async (resolve) => {
            const prevStoredPhotos = this.contact.photos || [];
